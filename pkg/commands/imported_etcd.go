...
func init() {
	...
	etcdDefragCmd.Flags().StringSliceVarP(&etcdCmdFlags.configFiles, "file", "f", nil, "specify config files or patches in a YAML file (can specify multiple)")
	etcdDefragCmd.
		PreRunE = etcdCmd.
		PreRunE
	...
	etcdLeaveCmd.
		Flags().StringSliceVarP(&etcdCmdFlags.
	...
	etcdMemberListCmd.PreRunE = etcdCmd.PreRunE
	etcdMemberRemoveCmd.Flags().
		StringSliceVarP(&etcdCmdFlags.
			configFiles, "file",
			"f", nil, "specify config files or patches in a YAML file (can specify multiple)",
		)
	etcdMemberRemoveCmd.
	...
	etcdSnapshotCmd.PreRunE = etcdCmd.
		PreRunE

	etcdStatusCmd.
		Flags().StringSliceVarP(&etcdCmdFlags.configFiles, "file",
		"f", nil, "specify config files or patches in a YAML file (can specify multiple)",
	)
	etcdStatusCmd.
		PreRunE = etcdCmd.
